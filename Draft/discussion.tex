...
However, there are instances where the constraints are implicit. For instance, consider the method description for \CodeIn{markSupported} method in \CodeIn{BufferInputStream} class in Java, that states ``\textit{Test if this input stream supports \CodeIn{mark}}''.
Although trivial for a human to interpret that the method \CodeIn{markSupported} must be invoked before the method \CodeIn{mark}, our approach is unable to infer such implicit temporal constraints.
